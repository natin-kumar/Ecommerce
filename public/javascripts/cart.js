...
function quantity(e,flag){
        ...
        .then(res=>{
            ...
            if(data.message=="Successful"){
                ...
                if(qty.value==0)
                {
                    ...
                    document.querySelector('.totalPrice').textContent=`Total Price: $${data.bill}`
                    let value=document.querySelector('.cartItems').innerText.substring(13);
                    value--;
                    document.querySelector('.cartItems').innerText=`Total Items: ${value}`;
                    const amt=finalAnmount-itemAmount
    ...
    function addAmount(amt,id){
        ...
        .then(data=>{
            console.log(data.cart.products,127);
            data.cart.products.forEach(ele=>{
                console.log(ele.productId);
            })
            const total=document.querySelector('.totalPrice');
            total.innerHTML=`Total Price: $${data.cart.bill}`;
        })
        .catch(err=>{
            console.log(err);
        })
